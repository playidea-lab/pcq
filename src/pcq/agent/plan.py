...
v4.0 ChangeOp 종류:
  - set_config: cq.yaml configs 의 키 갱신
  - set_parent_run_id / set_parent_run_path: lineage 메타데이터 주입
"""
...
# v4.0 에서 지원하는 ChangeOp 종류 — atom registry 제거 후 contract 변경만.
_SUPPORTED_OPS = {"set_config"}
...
class ValidationPolicy:
    """Plan 적용 후 validation 정책. v4.0 에서는 contract gate 만 적용된다."""
...
class ExperimentPlan:
    ...
    base 는 임의의 식별 정보를 담는 자유 dict (예: {"baseline_run": "runs/.../<id>"}).
    apply-plan 은 cq.yaml configs 만 수정 — train.py 는 사용자 영역.
    """

    schema_version: int = SCHEMA_VERSION
    id: str = ""                                          # e.g. "exp-001"
    intent: str = ""                                      # human-readable 설명
    base: dict[str, str] = field(default_factory=dict)    # free-form (baseline run path 등)
...
class ExperimentPlanSet:
    ...
    Fields:
      ...
      base: shared base mapping; each member plan may override.
